...
namespace LiveMarkdown.Controls
{
    public class MarkdownView : TemplatedControl, IDisposable
    {
        ...
        private void RenderParagraph(ParagraphBlock paragraph, Panel target, bool isTableCell = false)
        {
                    ...
                    switch (current)
                    {
                        ...
                        case CodeInline code:
                            inlines.Add(new Run
                            {
                                ...
                                Background = (IBrush?)Application.Current?.FindResource("MdInlineCodeBrush")
                            });
                            break;
            ...
            foreach (var item in list)
            {
                ...
                foreach (var childBlock in listItem)
                {
                    if (childBlock is ParagraphBlock paragraph)
                    {
                        ...
                        {
                            TextWrapping = TextWrapping.Wrap,
                            Margin = new Thickness(listDepth * 14, 2, 0, 2),
                            VerticalAlignment = VerticalAlignment.Center
                        };
                        ...
                        BuildInlinesWithOptionalHighlight(container, tb.Inlines, false);

                        target.Children.Add(tb);
                    }
                    else if (childBlock is ListBlock nestedList)
                    {
                        RenderListBlock(nestedList, target, listDepth + 1);
                    }
                    else
                    {
                        RenderBlock(childBlock, target, listDepth);
                    }
        ...
        private void RenderTable(MdTable table, Panel target)
        {
            ...
            foreach (MdTableRow row in table)
            {
                ...
                foreach (MdTableCell cell in row)
                {
                    ...
                    {
                        RenderParagraph(block, cellPanel, true);
                    }
